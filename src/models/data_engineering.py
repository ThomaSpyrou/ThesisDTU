...
def clean_target_output():
    true_labels = pd.read_csv('/home/tspy/workspace/thesis/ThesisDTU/data/estimation_generated/true_labels100.txt', header=None, names=['true_label'])
    predictions = pd.read_csv('/home/tspy/workspace/thesis/ThesisDTU/data/estimation_generated/predicted_labels100.txt', header = None, names=['prediction'])

    # softmax value needs thalpori
    cols = [i for i in range(100)]
    softmax_values = pd.read_csv('/home/tspy/workspace/thesis/ThesisDTU/data/estimation_generated/softmax_values100.txt', header=None, sep='\s+', names=cols)
    ...
    final_df.to_csv("./final100.csv", index=False)
